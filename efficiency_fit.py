...
def main(args):
    ...
    data_bincenter, residuals = fit_module.plot_fit_result(data_mc_np, (95, 104.97), PDF_eff, N_eff)
    plt.savefig("plot_efficiency_fit_result.png")
    plt.show()
    
    fig, ax, chi2_val, dof, p_value = analyze_residuals(residuals,
                      fig_file="plot_efficiency_residuals_with_gaussian.png", 
                      chi2_file="fit_efficiency_residuals.txt")

    print(f"Efficiency fit chi2: {chi2_val}, dof: {dof}, p value: {p_value}")
    plt.show()
